...
# NB! copy libs from CsoundLib64.framework/libs to eyeclick-server bundle ? perhaps in release bundle step?
}
...
#win32: QMAKE_POST_LINK += copy "$$shell_path($$PWD/eyeclick-server.bat)" "$$shell_path($$OUT_PWD/$$DESTDIR/)" # a workaround script for win8 and possibly other versions
...
macx {
    ...
    third.path = $$PWD
    third.commands = install_name_tool -change CsoundLib64.framework/Versions/6.0/CsoundLib64 @rpath/CsoundLib64.framework/Versions/6.0/CsoundLib64 $$OUT_PWD/$$DESTDIR/$${TARGET}.app/Contents/MacOS/eyeclick-server
